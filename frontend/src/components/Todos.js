  ...
  const updateTodoHandler = () => {
    const temp = todos.map((todo) => {
      if (todo.id !== inputId) {
        ...
        return {...todo, taskName: inputTaskName, description: inputDescription};
     }
   });
    setTodos(temp);
    toggleShowEditModal();
  ...
  return (
    <>
      <div className={!todos.length ? 'text-center mt-5' : 'visually-hidden'}>
        <img className='mt-5' src={todo} alt='todo' style={{filter: 'grayscale(100%)', opacity: '.2', width: '12%'}} />
      </div>
      {todos.map((todo) => (
        <MDBCard 
          key={todo.id} 
          ...
          <MDBCardBody className='mt-2' style={{display: 'flex', justifyContent: 'space-between', alignItems: 'baseline'}}>
            ...
            <div className='m-1 pe-4 ps-1' >
              ...
              <MDBBtn 
                ...
                className='m-1 ps-1' 
                style={{color: '#D94C4C'}}
                onClick={() => infoModalHandler(todo.id, todo.taskName, todo.description)}
              >
      ...
      {/* Modal Info */}
      <MDBModal tabIndex='-1' show={infoModal} getOpenState={(e) => setInfoModal(e)}>
          ...
          <MDBModalContent style={{backgroundColor: '#272833'}}>
            ...
            <div className='row px-5'>
              <div className='col-md'>
                  ...
                  {/* If there's an image */}
                  <div className='visually-hidden' style={{display: 'flex', justifyContent: 'center'}}>
                    <img className="w-50 my-3" src={todo} alt='todo' style={{borderRadius: '50%'}} />
                  </div>
            ...
            <MDBModalFooter>
              <MDBBtn style={{backgroundColor: '#C4C4C4', color: '#272833'}} onClick={toggleShowInfoModal}>
                Close
              </MDBBtn>
            </MDBModalFooter>
          </MDBModalContent>
      ...
      {/* Modal Edit */}
      <MDBModal tabIndex='-1' show={editModal} getOpenState={(e) => setEditModal(e)}>
        <MDBModalDialog centered size='lg'>
          <MDBModalContent style={{backgroundColor: '#272833'}}>
            ...
            <MDBModalFooter>
              <MDBBtn style={{backgroundColor: '#C4C4C4', color: '#272833'}} onClick={toggleShowEditModal}>
                Close
              </MDBBtn>
              <MDBBtn 
      ...
      {/* Modal Delete */}
      <MDBModal tabIndex='-1' show={deleteModal} getOpenState={(e) => setDeleteModal(e)}>
        <MDBModalDialog size='sm'>
          <MDBModalContent style={{backgroundColor: '#272833'}}>
            <MDBModalBody>
            <h4 className="text-center p-3" style={{color: '#D94C4C'}}>Are you sure?</h4>
            </MDBModalBody>
            <MDBModalFooter>
              <MDBBtn style={{backgroundColor: '#C4C4C4', color: '#272833'}} onClick={toggleShowDeleteModal}>
                Close
              </MDBBtn>
              <MDBBtn 
                style={{backgroundColor: '#D94C4C', color: '#272833'}}
